...
apache::vhost { 'hydroshare.local':
  server_name   => 'hydroshare.local',
  serveraliases => [
    'www.hydroshare.local'
  ],
  docroot       => '/hydroshare',
  port          => '80',
  env_variables => [
...
class { 'mysql::server':
  config_hash   => { 'root_password' => 'hydroshare_drupal' }
}

mysql::db { 'drupal_hydroshare':
  grant    => [
    'ALL'
  ],
  user     => 'admin',
  password => 'water',
  host     => 'localhost',
  sql      => '/hydroshare/sites/default/hydroshare.sql',
  charset  => 'utf8',
