function hydrology_model_plot( model_zip_path ){
  ...
  var svg = d3.select("#hydrology_model_vizualization").append("svg")
  ...
  var arr_data = [
    ...
    var data = arr_data.map(function(d){
      return {
        date: parseDate(d[0]),
        close: d[1]
      };
  ...
  x_context.domain(d3.extent(data, function(d) { return d.date; }));
  y_context.domain(d3.extent(data, function(d) { return d.close; }));
...
//  path  =  model_zip_path.replace(/\/[^\/]*$/, '/test_data_series.csv')
